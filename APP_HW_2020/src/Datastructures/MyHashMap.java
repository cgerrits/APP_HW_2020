...
import Datastructures.LinkedList.HANLinkedList;
...
public class MyHashMap<T> implements Iterable<T>{
     ...
     */
    private MyArrayList<HANLinkedList<Tuple<T>>> hashed;
    ...
    public void put(String key, T value) {
        int i = hash(key);
        HANLinkedList<Tuple<T>> list = hashed.get(i);
    ...
    public T lookup(String key) {
        HANLinkedList<Tuple<T>> list = hashed.get( hash(key) );
    ...
    public T[] toArray() {
        ...
        for (HANLinkedList<Tuple<T>> list : hashed) {
            int j = 0;
